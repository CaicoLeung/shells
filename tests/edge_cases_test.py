...
def test_run_git_command_raises_runtime_error_for_non_repo(
...
def test_cli_not_a_git_repository(tmp_path, monkeypatch) -> None:
    ...
    output = (
        result.stdout + str(result.exception) if result.exception else result.stdout
    )
    assert (
        "Error" in output or "Not a git repository" in output or result.exit_code == 1
    )
